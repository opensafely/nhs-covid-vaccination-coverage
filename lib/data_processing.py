...
def make_conditions( df,
                    dose_list = {"first":"covid_vacc_date",
                                "second":"covid_vacc_second_dose_date"},
                    vaccine_dates = {"covid_vacc_pfizerC_date" : "2022-12-22",
...
def load_adult_data(input_file='input_delivery.csv.gz', input_path="output", save_path={} ):
    """
    ...
    df = df.drop(columns=["imd","ethnicity_16", "ethnicity", 'ethnicity_6_sus',
       'ethnicity_16_sus', "has_follow_up"])
    ...
    for c in ["2nd_dose", "LD", "newly_shielded_since_feb_15", "dementia", 
         ...
         "solid_organ_transplantation", "chemo_or_radio", "intel_dis_incl_downs_syndrome","ssri",
          "lung_cancer", "cancer_excl_lung_and_haem", "haematological_cancer", "housebound", "ckd", "imid"]:
...
def load_child_data(input_file='input_delivery_u16.csv.gz', input_path="output", save_path={} ):
    """
    This reads in a csv that must be in output/ directory and cleans the
    data ready for use in the graphs and tables. The data are relevant for reporting
    ...
    df["covid_vacc_other_date"] = np.maximum( df['covid_vacc_oxford_date'].astype(str), df['covid_vacc_moderna_date'].astype(str) )
    df["covid_vacc_flag_other"] = np.where( (df['covid_vacc_oxford_date']!=0) | (df['covid_vacc_moderna_date'] != 0), 1, 0 )

    # Create a single field for brand of first and second dose
    # This excludes any uncertain cases where date of brand was too early or multiple brands were recorded
    choices = ["Pfizer (10 micrograms)", "Pfizer (30 micrograms)", "Oxford-AZ", "Moderna", "Unknown"]
    ...
    conditions_2nd = make_conditions(df,
        dose_list = {"first":"covid_vacc_date",
                    "second":"covid_vacc_second_dose_date"},
        vaccine_dates = {"covid_vacc_pfizerC_date" : "2021-12-22",
    ...
    conditions_3rd = make_conditions(df,
        dose_list = {"third":None},
        vaccine_dates = {"covid_vacc_third_dose_pfizerC_date" : None,
    ...
    for k, v in all_conditions.items():
        ...
        df[f'brand_of_{k}_dose'] = df[f'brand_of_{k}_dose'].str.replace(r'Oxford-AZ|Moderna', 'Other', regex=True)
    ...
    df = df.assign(    
        covid_vacc_pfizerA_pfizerC = np.where(
                                    ((df['brand_of_first_dose']=="Pfizer (30 micrograms)") & (df['brand_of_second_dose']=="Pfizer (10 micrograms)")) | (
                                        (df['brand_of_first_dose'] == "Pfizer (10 micrograms)") & (df['brand_of_second_dose'] == "Pfizer (30 micrograms)")), 1, 0),
        covid_vacc_other_pfizer=np.where(
            ( (df['brand_of_first_dose'].isin(non_pfizer_brands) ) & (df['brand_of_second_dose'].isin(pfizer_brands)) ) | (
                (df['brand_of_first_dose'].isin(pfizer_brands)) & (df['brand_of_second_dose'].isin(non_pfizer_brands))), 1, 0)
        )
